...
// Helper to get current device local ID
static int get_current_device_lid(void) {
  CUdevice device;
  CUresult result = cuCtxGetDevice(&device);
  if (result != CUDA_SUCCESS) {
    return -1;
  }
  return dev_pid_to_lid((int32_t)device);
}
...
void* chpl_gpu_impl_mem_array_alloc(size_t size) {
  ...
  if (chpl_gpu_arena_enabled()) {
    printf("chpl_gpu_impl_mem_array_alloc: trying arena alloc size=%zu\n", size);
    int dev_lid = get_current_device_lid();
    if (dev_lid >= 0) {
      void* ptr = chpl_gpu_arena_alloc(dev_lid, size);
      if (ptr != NULL) {
        return ptr;
      }
    }
  ...
  printf("chpl_gpu_impl_mem_array_alloc: falling back to standard CUDA alloc size=%zu\n", size);
...
void* chpl_gpu_impl_mem_alloc(size_t size) {
  // Note: Arena allocator is not used for non-array allocations.
  // Non-array allocations use host-pinned memory in ARRAY_ON_DEVICE mode.
#ifdef CHPL_GPU_MEM_STRATEGY_ARRAY_ON_DEVICE
...
void chpl_gpu_impl_mem_free(void* memAlloc) {
  if (memAlloc != NULL) {
    ...
    if (chpl_gpu_arena_is_ptr(memAlloc)) {
      chpl_gpu_arena_free(memAlloc);
      return;
    }
...
size_t chpl_gpu_impl_get_alloc_size(void* ptr) {
  // Check if this is an arena pointer (runtime check)
  if (chpl_gpu_arena_is_ptr(ptr)) {
    return chpl_gpu_arena_get_alloc_size(ptr);
  }
