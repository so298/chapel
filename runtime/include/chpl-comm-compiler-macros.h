...
static inline
void chpl_gen_comm_get_from_subloc_array_aware(void *addr, c_nodeid_t src_node,
                                   c_sublocid_t src_subloc, void* raddr,
                                   size_t size, int32_t commID, int ln,
                                   int32_t fn, int is_array)
{
  ...
  if (chpl_nodeID == src_node) {
    ...
    if (is_array) {
      chpl_gpu_comm_get_array(dst_subloc, addr, src_node, src_subloc, raddr,
                              size, commID, ln, fn);
    } else {  
        ...
        int ln, int32_t fn, int is_array)
{
  ...
  if (chpl_nodeID == dst_node) {
    ...
    if (is_array) {
      chpl_gpu_comm_put_array(dst_node, dst_subloc, raddr, src_subloc, addr,
                              size, commID, ln, fn);
    } else {  
